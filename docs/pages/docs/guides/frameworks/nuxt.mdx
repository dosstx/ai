...
It uses the `useChat` API from the `ai/vue` package to manage the state of the chat. It also uses the `handleSubmit` method to send messages to the endpoint and update the input field.
